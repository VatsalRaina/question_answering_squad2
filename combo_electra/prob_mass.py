...
def probability_mass(idx_mask, dist):
    return np.sum(dist[idx_mask.astype(bool)])
...
for ex in dev_data:
    ...
    # Get sum of probability mass at position of answer in the paragraph

    # Get indices of interest
    ans_idxs_mask = [0.0] * len(resp_words)
    for ans in answers:
        ans_tok = tokenizer.tokenize(ans)
        s_idx, e_idx = find_sub_list(ans_tok, resp_words)
        for idx in range(s_idx, e_idx+1):
            ans_idxs_mask[idx] = 1.0

    ans_idxs_mask = np.asarray(ans_idxs_mask)

    uni_dist = [1.0] * len(resp_words)
    uni_dist = np.asarray(uni_dist)
    uni_dist = uni_dist / np.sum(uni_dist)

    pm_saliency_true = probability_mass(ans_idxs_mask, saliency_max)
    pm_saliency_true_all.append(pm_saliency_true)
    print("Saliency Probability Mass:")
    print(pm_saliency_true)
    pm_uni_true = probability_mass(ans_idxs_mask, uni_dist)
    pm_uni_true_all.append(pm_uni_true)
    print("Uniform Probability Mass")
    print(pm_uni_true)

pm_saliency_true_all = np.array(pm_saliency_true_all)
pm_uni_true_all = np.array(pm_uni_true_all)

print("Saliency:")

r1 = np.mean(pm_saliency_true_all) 
print("Mean: ", r1) 
  
r2 = np.std(pm_saliency_true_all) 
print("std: ", r2) 
  
r3 = np.var(pm_saliency_true_all) 
print("variance: ", r3)

print("Uniform:")

r1 = np.mean(pm_uni_true_all) 
print("Mean: ", r1) 
  
r2 = np.std(pm_uni_true_all) 
print("std: ", r2) 
  
r3 = np.var(pm_uni_true_all) 
print("variance: ", r3)