...
def main(args):
    ...
    newBertConfig = BertConfig
    model = BertForQuestionAnswering.from_pretrained("bert-base-uncased")
    # Expand positional embeddings size (as default is only 512 tokens)
    new_position_embedding_size = 1024
    new_pos_embeddings = torch.nn.Embedding(new_position_embedding_size, model.bert.config.hidden_size)
    new_pos_embeddings.weight.data[:512] = model.bert.embeddings.position_embeddings.weight.data
    new_pos_embeddings.weight.data[512:] = model.bert.embeddings.position_embeddings.weight.data[-1][None,:].repeat(new_position_embedding_size-512,1)
    model.bert.embeddings.position_embeddings = new_pos_embeddings
    model.bert.config.max_position_embeddings = new_position_embedding_size
    model.config.max_position_embeddings = new_position_embedding_size
    print("Success")
    print(model.bert.config.max_position_embeddings)
    ...
    file_path = args.save_path+'bert_qa_seed'+str(args.seed)+'.pt'
