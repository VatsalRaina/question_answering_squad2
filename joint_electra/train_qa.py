...
def main(args):
    ...
    for ex in train_data:
        ...
        if start_idx==0:
            no_answer+=1
        # Get the overall position start and end index positions combined
        all_idx = start_idx + L_max * end_idx
        all_positions_true.append(all_idx)
    ...
    all_positions_true = torch.tensor(all_positions_true)
    all_positions_true = all_positions_true.long().to(device)
    ...
    train_data = TensorDataset(input_ids, all_positions_true, token_type_ids, attention_masks)
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            b_all_pos_true = batch[1].to(device)
            b_tok_typ_ids = batch[2].to(device)
            b_att_msks = batch[3].to(device)
            model.zero_grad()
    ...
    file_path = args.save_path+'electra_jointQA_seed'+str(args.seed)+'.pt'
