...
class ElectraQA(torch.nn.Module):
    def __init__(self):
        ...
        self.classifierA = ElectraClassificationHead()
        self.classifierB = ElectraClassificationHead()
        self.classifierC = ElectraClassificationHead()
        self.dropout = torch.nn.Dropout(self.electra.config.hidden_dropout_prob)
        # self.init_weights()
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        verification_logitsA = torch.sigmoid(self.classifierA(sequence_output))
        verification_logitsB = torch.sigmoid(self.classifierB(sequence_output))
        verification_logitsC = torch.sigmoid(self.classifierC(sequence_output))

        verification_logits = (verification_logitsA + verification_logitsB + verification_logitsC) / 3
