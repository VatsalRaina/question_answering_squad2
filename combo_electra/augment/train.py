...
parser.add_argument('--train_path', type=str, help='Load path to training dataset')
...
def main(args):
    ...
    for ex in train_data:
        ...
        # if count==3:
        #    break
        question, passage, aug_passage = ex["question"], ex["context"], ex["aug_context"]
        combo = question + " [SEP] " + passage
        combo_aug = question + " [SEP] " + aug_passage
        inp_ids = tokenizer.encode(combo)
        inp_ids_aug = tokenizer.encode(combo_aug)
        if len(inp_ids) > 512:
            too_long+=1
            continue
            print(ex)
